wp.customize.controlConstructor['cream-blog-typography'] = wp.customize.Control.extend({
    ...
    ready: function () {
        ...
        let fontFamilySelectEle = control.container.find('#cream-blog-font-family-select-' + control.id);
        if (fontFamilySelectEle.length > 0) {
            new SlimSelect({
                select: '#cream-blog-font-family-select-' + control.id,
                showSearch: true,
        ...
        let fontVariantsSelectEle = control.container.find('#cream-blog-font-variants-select-' + control.id);
        if (fontVariantsSelectEle.length > 0) {
            new SlimSelect({
                select: '#cream-blog-font-variants-select-' + control.id,
                showSearch: false,
                contentPosition: 'relative',
                settings: {
                    openPosition: 'down' // 'auto', 'up' or 'down'
        ...
        let fontWeightSelectEle = control.container.find('#cream-blog-font-weight-select-' + control.id);
        if (fontWeightSelectEle.length > 0) {
            new SlimSelect({
                select: '#cream-blog-font-weight-select-' + control.id,
                showSearch: false,
                contentPosition: 'relative',
                settings: {
                    openPosition: 'down' // 'auto', 'up' or 'down'
        ...
        fontFamilySelectEle.on('change', function () {
            ...
            let fontVariantsSelectWrapperEle = jQuery('#cream-blog-google-variants-select-wrapper-' + thisFontFamilySelectEle.data('control'));
        ...
        body.on('change', '#cream-blog-font-family-select-' + control.id, function (event) {
            ...
            if (fontSource === 'google') {
                ...
                let fontURLEle = jQuery('#cream-blog-font-url-' + control.id);
        ...
        body.on('change', '#cream-blog-font-variants-select-' + control.id, function (event) {
            event.preventDefault();
            let thisEle = jQuery(this);
        ...
        body.on('change', '#cream-blog-font-weight-select-' + control.id, function (event) {
            event.preventDefault();
            let thisEle = jQuery(this);
        ...
        devices.forEach(function (device) {
            ...
            // On font-size change.
            body.on('change', '#cream-blog-' + device + '-font-size-' + control.id, function (event) {
                event.preventDefault();
                let thisEle = jQuery(this);
                let settingValue = getSettingValue(control.id);
            ...
            body.on('change', '#cream-blog-' + device + '-font-size-unit-input-' + control.id, function (event) {
                event.preventDefault();
                let thisEle = jQuery(this);
                let settingValue = getSettingValue(control.id);
                let targetDevice = thisEle.data('device');

                let fontSizeInputEle = jQuery('#cream-blog-' + targetDevice + '-font-size-' + control.id);
            ...
            body.on('change', '#cream-blog-' + device + '-line-height-' + control.id, function (event) {
                event.preventDefault();
                let thisEle = jQuery(this);
                let settingValue = getSettingValue(control.id);
                let targetDevice = thisEle.data('device');
            ...
            body.on('change', '#cream-blog-' + device + '-letter-spacing-' + control.id, function (event) {
                event.preventDefault();
                let thisEle = jQuery(this);
                let settingValue = getSettingValue(control.id);
                let targetDevice = thisEle.data('device');
            ...
            body.on('change', '#cream-blog-' + device + '-letter-spacing-unit-input-' + control.id, function (event) {
                event.preventDefault();
                let thisEle = jQuery(this);
                let settingValue = getSettingValue(control.id);
                let targetDevice = thisEle.data('device');

                let letterSpacingInputEle = jQuery('#cream-blog-' + targetDevice + '-letter-spacing-' + control.id);
        ...
        body.on('change', '#cream-blog-typography-font-style-input-' + control.id, function (event) {
            event.preventDefault();
            let thisEle = jQuery(this);
            let settingValue = getSettingValue(control.id);
        ...
        body.on('change', '#cream-blog-typography-text-transform-input-' + control.id, function (event) {
            event.preventDefault();
            let thisEle = jQuery(this);
            let settingValue = getSettingValue(control.id);
        ...
        let fontSizeSliders = [
            control.container.find('.cream-blog-slider.cream-blog-slider-font-size.desktop-slider'),
            control.container.find('.cream-blog-slider.cream-blog-slider-font-size.tablet-slider'),
            control.container.find('.cream-blog-slider.cream-blog-slider-font-size.mobile-slider'),
            control.container.find('.cream-blog-slider.cream-blog-slider-font-size.normal-slider'),
            control.container.find('.cream-blog-slider.cream-blog-slider-line-height.desktop-slider'),
            control.container.find('.cream-blog-slider.cream-blog-slider-line-height.tablet-slider'),
            control.container.find('.cream-blog-slider.cream-blog-slider-line-height.mobile-slider'),
            control.container.find('.cream-blog-slider.cream-blog-slider-line-height.normal-slider'),
            control.container.find('.cream-blog-slider.cream-blog-slider-letter-spacing.desktop-slider'),
            control.container.find('.cream-blog-slider.cream-blog-slider-letter-spacing.tablet-slider'),
            control.container.find('.cream-blog-slider.cream-blog-slider-letter-spacing.mobile-slider'),
            control.container.find('.cream-blog-slider.cream-blog-slider-letter-spacing.normal-slider')
        ];

        // Initialize sliders using jQuery slider UI.
        fontSizeSliders.forEach(function (fontSizeSlider) {
            let fontSizeSliderInput = fontSizeSlider.next('.cream-blog-slider-input').find('input.slider-input');
        ...
        body.on('click', '#cream-blog-text-style-reset-' + control.id, function (event) {
            event.preventDefault();
            // Reset text-style input value.
            let textStyleInputEle = jQuery('#cream-blog-typography-font-style-input-' + control.id);
            ...
            let textStyleButton = jQuery('#cream-blog-font-style-' + textStyleDefaultValue + '-' + control.id);
            textStyleButton.addClass('active').siblings().removeClass('active');
        });

        // On text-transform reset event.
        body.on('click', '#cream-blog-text-transform-reset-' + control.id, function (event) {
            event.preventDefault();
            // Reset text-transform input value.
            let textTransformInputEle = jQuery('#cream-blog-typography-text-transform-input-' + control.id);
            ...
            let textTransformButton = jQuery('#cream-blog-text-transform-' + textTransformDefaultValue + '-' + control.id);
        ...
        function saveSetting(controlID, newSettingValue) {
            let settingInputEle = jQuery('#cream-blog-typography-control-value-' + controlID);
